...
- `uv run nox -s precommit`: runs all prek/pre-commit hooks, except
  `no-commit-to-branch` (it exists to stop a direct commit to `main`, which
  isn't what this session is for). Run it explicitly with
  `uv run prek run no-commit-to-branch --all-files` if you need to check it.
