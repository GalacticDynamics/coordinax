...
This page covers contribution expectations and pull request readiness. For the practical local development workflow, including environment setup and the day-to-day `uv` and `nox` commands used in this repository, see [dev.md](dev.md). Maintainer-only operational tasks are summarized separately in [maintainers.md](maintainers.md).
...
So you are interested in contributing to `coordinax`? Excellent! We love contributions! `coordinax` is open source, built on open source, and we'd love to have you hang out in our community.

## How to Contribute, Best Practices

Most contributions to `coordinax` are done via [pull requests](https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/about-pull-requests) from GitHub users' forks of the [`coordinax` repository](https://github.com/GalacticDynamics/coordinax). If you are new to this style of development, check out `astropy`'s [development workflow](https://docs.astropy.org/en/latest/development/quickstart.html).
...
- **Performance improvements**: if you are making changes that impact `coordinax` performance, consider adding a performance benchmark in `tests`. A maintainer will also be able to run comparative benchmarks to catch performance changes. The PR needs to have the `run-benchmarks` label to run the workflow.
...
For smaller changes, it is fine to iterate with narrower checks first, as described in [dev.md](dev.md), before running the broader gate.
...
- Does the implementation match the relevant spec in `docs/spec.md` or the package-local `docs/spec.md`?
