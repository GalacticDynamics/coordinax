...
In `coordinax`, this is represented by `cx.r.EmbeddedManifold` with `chart_kind`
...
The orthonormal tangent frame (columns of `frame_to_cart`) is:
...
`embed_pos` maps intrinsic coordinates to ambient coordinates. `project_pos`
...
At the poles (`sin(theta)=0`), longitude is not unique; `project_pos` sets
`phi = 0` by convention.
...
`embed_dif` and `project_dif` operate on physical components (not coordinate
derivatives) using the orthonormal frame from `frame_to_cart`:
...
p = {"theta": u.Angle(jnp.pi / 2, "rad"), "phi": u.Angle(0.0, "rad")}
v = {"theta": u.Quantity(1.0, "km/s"), "phi": u.Quantity(0.0, "km/s")}
...
`frame_to_cart` provides the orthonormal frame in ambient Cartesian components.
