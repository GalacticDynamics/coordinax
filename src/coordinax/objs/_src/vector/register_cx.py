...
def as_disp(
    ...
    **Intrinsic manifold case:**
    ...
    >>> disp_sph = cx.as_disp(pos_sph, origin, chart=cxc.cyl3d, at=pos_sph)
    >>> disp_sph.chart
    Cylindrical3D()
    ...
    if origin is None:
        disp_data, disp_chart = cxapi.as_disp(point.data, point.chart, origin)
        ...
        disp_data, disp_chart = cxapi.as_disp(point.data, point.chart, origin.data)
...
def apply_op(
    ...
    **kw: Any,
    ...
    >>> v_rot = cxop.apply_op(op, None, v)  # no time dependence
    >>> print(v_rot)
    <Vector: chart=Cart3D, role=Point (x, y, z) [m]
        [-2.  1.  3.]>
    ...
    result_data = cxapi.apply_op(op, tau, v.role, v.chart, v.data, **(other_kw | ats))
