...
class AbstractVectorLike(
    ...
    def is_like(cls, obj: Any, /) -> TypeIs["Self"]:
        ...
        >>> vec = cx.Vector.from_([1, 2, 3], "m")
        >>> cx.objs.AbstractVectorLike.is_like(vec)
        True

        >>> space = cx.PointedVector.from_({"base": vec})
        >>> cx.objs.AbstractVectorLike.is_like(space)
        True

        >>> import coordinax.frames as cxf
        >>> coord = cx.Coordinate(vec, cxf.ICRS())
        >>> cx.objs.AbstractVectorLike.is_like(coord)
        True

        >>> cx.objs.AbstractVectorLike.is_like(42)
        False
