...
class PointedVector(
    ...
    **fields : Vector
    ...
    def chart(self) -> cxc.CartesianProductChart:
        ...
        >>> bundle = cx.PointedVector(base=base, velocity=vel)
        >>> bundle.chart
        CartesianProductChart(factors=(Cart3D(), Cart3D()),
                              factor_names=('base', 'velocity'))
...
def apply_op(
    ...
    **kw: Any,
    ...
    try:
        new_base = cxapi.apply_op(op, tau, fp.base, **kw)
        applied = True
    ...
    for name, vec in fp._data.items():
        try:
            ...
            new_fields[name] = cxapi.apply_op(op, tau, vec, at=at_vec, **kw)
            applied = True
        except (NotFoundLookupError, TypeError):
            new_fields[name] = vec
