...
def uconvert(usys: u.AbstractUnitSystem, vec: Vector, /) -> Vector:
    ...
    return Vector(data, chart=vec.chart, rep=vec.rep)
...
def uconvert(
    ...
    <Vector: chart=Cart2D, rep=point (x, y) [km]
        [1.e-03 2.e+00]>
    ...
    ...     chart=cxc.sph3d, rep=cxr.point)
    >>> print(sph.uconvert({u.dimension("length"): "km", u.dimension("angle"): "deg"}))
    <Vector: chart=Spherical3D, rep=point (r[km], theta[deg], phi[deg])
...
def uconvert(units: Mapping[str, Any], vec: Vector, /) -> Vector:
    ...
    <Vector: chart=Cart2D, rep=point (x[km], y[m])
        [1.e-03 2.e+03]>
    ...
    <Vector: chart=Spherical3D, rep=point (r[km], theta[rad], phi[rad])
...
def uconvert(flag: Literal[ToUnitsOptions.consistent], vec: Vector, /) -> Vector:
    ...
    --------
    >>> import unxt as u
    >>> import coordinax.vectors as cxobjs

    We can convert a vector to the given units:

    >>> cart = cxobjs.Vector.from_([1, 2, 3], "m")

    If all you want is to convert to consistent units, you can use
    ``"consistent"``:

    >>> print(cart.uconvert(cxobjs.ToUnitsOptions.consistent))
    <Vector: chart=Cart3D, rep=point (x, y, z) [m]
        [1 2 3]>

    >>> sph = cart.vconvert(cxc.sph3d)
    >>> print(sph.uconvert(cxobjs.ToUnitsOptions.consistent))
