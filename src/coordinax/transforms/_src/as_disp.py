"""Role transformation: as_disp (to Pos role)."""
...
def as_disp(
    ...
    *,
    ...
    >>> cx.as_disp({"x": 1, "y": 2, "z": 3}, cxc.cart3d, None)
    ({'x': 1, 'y': 2, 'z': 3}, Cart3D())
    ...
    if not chart.is_euclidean:
        msg = "as_disp is currently only implemented for charts with Euclidean metric."
        raise NotImplementedError(msg)

    # Convert to Cartesian chart
    cart_chart = chart.cartesian
    point_cart = cxapi.point_transform(cart_chart, chart, point, usys=usys)
...
def as_disp(
    ...
    *,
    ...
    >>> cx.as_disp({"x": 2}, cxc.cart1d, {"x": 0.5})
    ...
    # Point Transform
    point_cart = cxapi.point_transform(cart_chart, chart, point, usys=usys)
    origin_cart = cxapi.point_transform(cart_chart, chart, origin, usys=usys)
...
def as_disp(
    ...
    *,
    ...
    """
    msg = "Embedded manifolds require an explicit origin for as_disp."
...
def as_disp(
    ...
    *,
    ...
    manifold (e.g. longitude/latitude). ``as_disp`` embeds both into the ambient
    ...
    >>> embed_twosphere = cxe.EmbeddedManifold(cxc.twosphere, cxc.cart3d,
    ...                                        params={"R": u.Q(2.0, "km")})
    ...
    >>> d, ambient = cx.as_disp(p, embed_twosphere, o)
    ...
    point_ambient = cxapi.embed_point(chart, point, usys=usys)
    origin_ambient = cxapi.embed_point(chart, origin, usys=usys)

    # Use ambient chart as basis for as_disp
    disp_data, disp_chart = cxapi.as_disp(
