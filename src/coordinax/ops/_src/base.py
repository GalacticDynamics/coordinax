...
class AbstractOperator(eqx.Module):
    ...
    def __call__(self: "AbstractOperator", tau: Any, x: Any, /, **kwargs: Any) -> Any:
        ...
        """
        return cxapi.apply_op(self, tau, x, **kwargs)
    ...
    def __call__(self: "AbstractOperator", x: Any, /, **kwargs: Any) -> Any:
        ...
        """
        return cxapi.apply_op(self, None, x, **kwargs)
    ...
    def simplify(self) -> "AbstractOperator":
        ...
        >>> pipe = cxop.Pipe((cxop.Identity(), cxop.Identity()))
        >>> pipe
        Pipe((Identity(), Identity()))
        >>> pipe.simplify()
        Identity()

        >>> op = cxop.GalileanOp(translation=u.Q([0., 2., 3.], "km"))
        >>> op.simplify()
        Translate(
    ...
    def __str__(self) -> str:
        ...
        >>> op = cxop.Rotate([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        >>> print(op)
        Rotate([[1 0 0]
                [0 1 0]
                [0 0 1]])
        ...
        GalileanOp(
            ...
            velocity=Boost(
                {'x': Q(1., 'km / s'), 'y': Q(2., 'km / s'), 'z': Q(3., 'km / s')},
                chart=Cart3D()
            )
        )
    ...
    def __or__(self, other: "AbstractOperator", /) -> "coordinax.ops.Pipe":
        ...
        >>> op1 = cxop.Rotate([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        >>> op2 = cxop.Identity()
        >>> op3 = op1 | op2
        >>> op3
        Pipe((Rotate(i64[3,3](jax)), Identity()))

        >>> op4 = cxop.Identity() | op3
        >>> op4
...
def from_(cls: type[AbstractOperator], obj: Mapping[str, Any], /) -> AbstractOperator:
    ...
    --------
    >>> import coordinax.ops as cxop
    >>> cxop.Pipe.from_({"operators": (cxop.Identity(), cxop.Identity())})
    Pipe((Identity(), Identity()))
...
def from_(
    ...
    >>> op = cxop.Translate.from_([1, 1, 1], "km")
    >>> print(op)
    Translate({'x': Q(1, 'km'), 'y': Q(1, 'km'), 'z': Q(1, 'km')}, chart=Cart3D())

    >>> op = cxop.Boost.from_([1, 1, 1], "km/s")
    >>> print(op)
    Boost(
        {'x': Q(1, 'km / s'), 'y': Q(1, 'km / s'), 'z': Q(1, 'km / s')},
        chart=Cart3D()
    )
...
def from_(cls: type[AbstractOperator], obj: AbstractOperator, /) -> AbstractOperator:
    ...
    >>> op = cxop.Identity()
    >>> cxop.Identity.from_(op) is op
    True

    If the object is a different type, it will error.

    >>> try:
    ...     cxop.Translate.from_(op)
    ...
    >>> class MyOperator(cxop.Identity):
    ...     pass
    ...
    >>> newop = cxop.Identity.from_(op)
    >>> newop is op, isinstance(newop, cxop.Identity)
    (False, True)
...
def eval_op(op: OpT, tau: Any, /) -> OpT:
    ...
    **Time-dependent operator:**
    ...
    >>> op = cxop.Translate(lambda t: cx.cdict(u.Q([t.ustrip("s"), 0, 0], "km")),
    ...                    chart=cxc.cart3d)
    >>> tau = u.Q(5.0, "s")
    >>> op_eval = cxop.eval_op(op, tau)
    >>> op_eval.delta["x"]
    Quantity(Array(5., dtype=float64), unit='km')

    **Static operator (no change):**

    >>> op_static = cxop.Translate(cx.cdict(u.Q([1, 2, 3], "km")), chart=cxc.cart3d)
    >>> op_eval_static = cxop.eval_op(op_static, tau)
    >>> op_eval_static.delta["x"]
    Quantity(Array(1, dtype=int64), unit='km')

    **Identity operator:**

    >>> identity = cxop.Identity()
    >>> cxop.eval_op(identity, tau)
    Identity()
