...
class GalileanOp(AbstractCompositeOperator):
    ...
    >>> op = cxop.GalileanOp(
    ...     translation=cxop.Translate.from_([2., 3., 4.], "km"),
    ...     velocity=cxop.Boost.from_([1., 2., 3.], "km/s"),
    ... )
    >>> op
    GalileanOp(
      Translate(
          {'x': Q(f64[], 'km'), 'y': Q(f64[], 'km'), 'z': Q(f64[], 'km')},
          chart=Cart3D()
      ),
      Boost(
...
def apply_op(
    ...
    >>> cxop.apply_op(op, tau, q)
    Quantity(Array([2., 3., 4.], dtype=float64), unit='km')

    """
    result = x
    for sub_op in op.operators:
        result = cxapi.apply_op(sub_op, tau, result, **kwargs)
    return result
...
def simplify(op: GalileanOp, /, **kwargs: Any) -> SimplifyOpR:
    ...
    GalileanOp(
      ...
      Translate( {'x': Q(f64[], 'km'), 'y': Q(f64[], 'km'), 'z': Q(f64[], 'km')},
                chart=Cart3D() ),
      Boost(
        {'x': Q(f64[], 'km / s'), 'y': Q(f64[], 'km / s'), 'z': Q(f64[], 'km / s')},
        chart=Cart3D() )
    ...
    """
    simple_ops = [cxapi.simplify(x, **kwargs) for x in op.operators]
    if any(
...
def from_(cls: type[GalileanOp], q: u.AbstractQuantity, /) -> GalileanOp:
    ...
    >>> cxop.GalileanOp.from_(u.Q([1, 2, 3], "km"))
    GalileanOp(
    ...
    >>> cxop.GalileanOp.from_(u.Q([100, 0, 0], "km/s"))
    GalileanOp(
      Boost(
          {'x': Q(i64[], 'km / s'), 'y': Q(i64[], 'km / s'), 'z': Q(i64[], 'km / s')},
          chart=Cart3D()
      )
    )
