...
class Pipe(AbstractCompositeOperator):
    ...
    >>> boost = cxop.Boost.from_([1, 2, 3], "km/s")
    >>> pipe = cxop.Pipe((shift, boost))
    >>> pipe
...
def apply_op(
    ...
    **kw: object,
    ...
    >>> shift = cxop.Translate.from_([1, 2, 3], "km")
    >>> pipe = cxop.Pipe((shift,))
    >>> data = {"x": u.Q(0, "km"), "y": u.Q(0, "km"), "z": u.Q(0, "km")}
    >>> cxop.apply_op(pipe, None, data, role=cxr.Point)
    {'x': Quantity..., 'y': Quantity..., 'z': Quantity...}

    """
    result = x
    for sub_op in op.operators:
        result = cxapi.apply_op(sub_op, tau, role, chart, result, **kw)
    return result
...
def apply_op(
    ...
    >>> pipe = cxop.Pipe((shift,))
    >>> q = u.Q([0, 0, 0], "km")
    >>> cxop.apply_op(pipe, None, q)
    Quantity(Array([1, 2, 3], dtype=int64), unit='km')

    """
    result = x
    for sub_op in op.operators:
        result = cxapi.apply_op(sub_op, tau, result, **kw)
    return result
