...
from coordinax._src.custom_types import HasShape, OptUSys
from coordinax._src.utils import pack_uniform_unit, unpack_with_unit
...
class Rotate(AbstractOperator):
    ...
    >>> v_pos = cx.as_disp(v)  # A cxr.PhysDisp vector
    >>> print(op(t, v_pos))
    ...
    >>> nq = cxop.apply_op(op, t, cxr.phys_vel, cxc.cart3d, q)  # role & chart
    >>> wl.pprint(nq, short_arrays="compact", use_short_name=True)
    ...
    def from_euler(
        ...
        >>> op = cxop.Rotate.from_euler("z", u.Q(90, "deg"))
        >>> op.R.round(2)
        Array([[ 0., -1.,  0.],
               [ 1.,  0.,  0.],
               [ 0.,  0.,  1.]], dtype=float64)
...
def from_(cls: type[Rotate], obj: Rotate, /) -> Rotate:
    ...
    >>> R = cxop.Rotate(jnp.eye(3))
    >>> cxop.Rotate.from_(R) is R
    True
...
def from_(cls: type[Rotate], obj: AbcQ, /) -> Rotate:
    ...
    >>> cxop.Rotate.from_(u.Q(jnp.eye(3), ""))
    Rotate(f64[3,3](jax))
...
def simplify(op: Rotate, /, **kw: Any) -> AbstractOperator:
    ...
    >>> op = (  cxop.Rotate.from_euler("z", u.Q(45, "deg"))
    ...       @ cxop.Rotate.from_euler("z", u.Q(-45, "deg")))
    >>> cxop.simplify(op)
    Identity()
...
def apply_op(
    ...
    **kw: Any,
    ...
    p_cart = cxapi.point_transform(cart, chart, x, usys=usys)
    ...
    return cxapi.point_transform(chart, cart, p_cart_rot, usys=usys)
...
def apply_op(
    ...
    **kw: Any,
    ...
    at_rot = (
        {}
        if at is None
        else cxapi.apply_op(op, tau, cxr.point, chart, at=at, usys=usys)
    )
    ...
    # B (..., N, n)
    B = cxapi.frame_cart(chart, at=at0, usys=usys)
    ...
    B_rot = cxapi.frame_cart(chart, at=at_rot, usys=usys)
    g = cxapi.metric_of(chart)
...
def apply_op(
    ...
    **kw: Any,
    ...
    x_cart_dict = cxapi.point_transform(cart, chart, at0, usys=usys)
...
def apply_op(
    ...
    **kw: Any,
    ...
    def _maybe(
        ...
        # Apply rotation
        return cxapi.apply_op(op_eval, tau, role, factor_chart, part, usys=usys, **ats)
