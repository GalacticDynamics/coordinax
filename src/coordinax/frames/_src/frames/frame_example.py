...
class Alice(AbstractReferenceFrame):
    ...
    >>> op = cxf.point_frame_transform(alice, bob)
    >>> print(op)
    Pipe((
      Translate(...),
      Boost(...)
...
def point_frame_transform(
    ...
    >>> alice = cxf.Alice()
    >>> cxf.point_frame_transform(alice, alice)
    Identity()

    >>> friend = cxf.FriendOfAlice()
    >>> cxf.point_frame_transform(friend, friend)
    Identity()

    >>> bob = cxf.Bob()
    >>> cxf.point_frame_transform(bob, bob)
    Identity()
...
@plum.dispatch
def point_frame_transform(from_frame: Alice, to_frame: FriendOfAlice, /) -> cxop.Pipe:
    ...
    >>> op = cxf.point_frame_transform(alice, friend)
    >>> print(op)
    Pipe((
      Translate(...),
      Rotate(...)
...
@plum.dispatch
def point_frame_transform(from_frame: Alice, to_frame: Bob, /) -> cxop.Pipe:
...
def point_frame_transform(
    ...
    >>> op = cxf.point_frame_transform(friend, alice)
    >>> print(op)
    Pipe((
      Rotate(...),
      Translate(...)
    ...
    """
    return api.point_frame_transform(to_frame, from_frame).inverse  # pylint: disable=W1114
