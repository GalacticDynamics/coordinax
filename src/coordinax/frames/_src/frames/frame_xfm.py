...
class TransformedReferenceFrame(AbstractReferenceFrame, Generic[FrameT]):
    ...
    **frames**, not the coordinates. This is a passive (intrinsic) versus active
    ...
    >>> R = cxop.Rotate(jnp.asarray([[0., -1, 0], [1, 0, 0], [0, 0, 1]]))
    ...
    >>> op = cxf.point_frame_transform(cxf.ICRS(), frame)
    ...
    xop: cxop.AbstractOperator


@plum.dispatch
def point_frame_transform(
    from_frame: AbstractReferenceFrame, to_frame: TransformedReferenceFrame
) -> cxop.AbstractOperator:
    ...
    """
    return (
        api.point_frame_transform(from_frame, to_frame.base_frame)
...
def point_frame_transform(
    ...
    >>> op = cxf.point_frame_transform(frame, cxf.ICRS())
    ...
    """
    return from_frame.xop | api.point_frame_transform(from_frame.base_frame, to_frame)
...
def point_frame_transform(
    ...
    >>> shift = cxop.Translate.from_([1, 0, 0], "kpc")
    >>> frame2 = cxf.TransformedReferenceFrame(frame1, shift)

    >>> op1to2 = cxf.point_frame_transform(frame1, frame2)
    ...
    return (
        from_frame.xop
        | api.point_frame_transform(from_frame.base_frame, to_frame.base_frame)
        | to_frame.xop.inverse
    )
