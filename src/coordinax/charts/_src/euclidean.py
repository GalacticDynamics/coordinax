...
from coordinax._src.custom_types import Ang, Ds, Ks, Len, Spd
from coordinax.api import CDict, CsDict
...
class SpaceTimeEuclidean(AbstractFlatCartesianProductChart[Ks, Ds]):
    ...
    >>> import unxt as u
    >>> rep = cxc.SpaceTimeEuclidean(cxc.cart3d)
    >>> p = {"ct": u.Q(1.0, "km"), "x": u.Q(0.0, "km"), "y": u.Q(0.0, "km"),
    ...      "z": u.Q(0.0, "km")}
    >>> cxm.metric_of(rep).metric_matrix(rep, p).shape
    (4, 4)
...
def cartesian_chart(obj: SpaceTimeEuclidean, /) -> SpaceTimeEuclidean: # type: ignore[type-arg]
    ...
    >>> chart = cxc.SpaceTimeEuclidean(cxc.sph3d)
    >>> chart
