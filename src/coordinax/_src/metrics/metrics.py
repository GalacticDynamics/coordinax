...
class AbstractMetric(metaclass=abc.ABCMeta):
    ...
    >>> import unxt as u
    >>> metric = cx.metrics.metric_of(cx.charts.cart3d)
    >>> p = {"x": u.Q(1.0, "km"), "y": u.Q(0.0, "km"),
    ...      "z": u.Q(0.0, "km")}
    >>> metric.metric_matrix(cx.charts.cart3d, p).shape
    (3, 3)
...
class EuclideanMetric(AbstractMetric):
    ...
    Notes
    -----
    - This metric is used for Euclidean reps exposed in ``cx.charts.cart3d``.
