...
@CartesianPos3D.constructor._f.dispatch  # type: ignore[attr-defined, misc]  # noqa: SLF001
def constructor(
...
class CartesianVel3D(AvalMixin, AbstractVel3D):
    ...
    def norm(self, _: AbstractPos3D | None = None, /) -> ct.BatchableSpeed:
        ...
        >>> from unxt import Quantity
        >>> import coordinax as cx
        >>> c = cx.CartesianVel3D.constructor([1, 2, 3], "km/s")
        >>> c.norm()
...
@CartesianVel3D.constructor._f.dispatch  # type: ignore[attr-defined,misc]  # noqa: SLF001
def constructor(
...
def _add_pp(lhs: CartesianVel3D, rhs: CartesianVel3D, /) -> CartesianVel3D:
    ...
    --------
    >>> import coordinax as cx
    >>> q = cx.CartesianVel3D.constructor([1, 2, 3], "km/s")
    >>> q2 = q + q
    >>> q2.d_y
...
def _sub_v3_v3(lhs: CartesianVel3D, other: CartesianVel3D, /) -> CartesianVel3D:
    ...
    >>> q = CartesianVel3D.constructor(Quantity([1, 2, 3], "km/s"))
    >>> q2 = q - q
    >>> q2.d_y
...
class CartesianAcc3D(AvalMixin, AbstractAcc3D):
    ...
    def norm(self, _: AbstractVel3D | None = None, /) -> ct.BatchableAcc:
        ...
        >>> from unxt import Quantity
        >>> import coordinax as cx
        >>> c = cx.CartesianAcc3D.constructor([1, 2, 3], "km/s2")
        >>> c.norm()
...
@CartesianAcc3D.constructor._f.dispatch  # type: ignore[attr-defined, misc]  # noqa: SLF001
def constructor(
...
def _mul_ac3(lhs: ArrayLike, rhs: CartesianPos3D, /) -> CartesianPos3D:
    ...
    >>> v = cx.CartesianPos3D.constructor([1, 2, 3], "kpc")
    >>> jnp.multiply(2, v).x
    Quantity['length'](Array(2., dtype=float32), unit='kpc')
