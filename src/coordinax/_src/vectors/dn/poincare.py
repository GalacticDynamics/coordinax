...
class PoincarePolarVector(AbstractPos):  # TODO: better name
    ...
    rho: BBtLength = eqx.field(converter=u.Q["length"].from_)
    r"""Cylindrical radial distance :math:`\rho \in [0,+\infty)`."""

    pp_phi: Real[u.Q[dims.L / dims.T**0.5], "*#batch"] = eqx.field()
    r"""Poincare phi-like variable."""

    z: BBtLength = eqx.field(converter=u.Q["length"].from_)
    r"""Height :math:`z \in (-\infty,+\infty)`."""

    dt_rho: ct.BBtSpeed = eqx.field(converter=u.Q["speed"].from_)
    r"""Cyindrical radial speed :math:`d\rho/dt \in [-\infty, \infty]."""

    dt_pp_phi: Real[u.Q[dims.L / dims.T**0.5], "*#batch"] = eqx.field()
    r"""Poincare phi-like velocity variable."""

    dt_z: ct.BBtSpeed = eqx.field(converter=u.Q["speed"].from_)
