...
class AstropyRepresentationAPIMixin:
    ...
    def represent_as(self, target: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        target : type[`coordinax.Vector`]
            The representation type to convert to, e.g. `cx.r.sph3d`.
        *args, **kwargs
        ...
        >>> q_sph = q_cart.represent_as(cx.r.sph3d)
        >>> print(q_sph)
        ...
        >>> q_cart = cx.Vector.from_([1, 2, 3], "m")
        >>> v_sph = v_cart.represent_as(cx.r.sph3d, q_cart)
        >>> print(v_sph)
        <Vector: rep=Spherical3D, role=Vel, metric=EuclideanMetric(n=3) (r, theta, phi) [m / s]
        ...
        >>> a_sph = a_cart.represent_as(cx.r.sph3d, q_cart)
        >>> print(a_sph)
        <Vector: rep=Spherical3D, role=Acc, metric=EuclideanMetric(n=3) (r, theta, phi) [m / s2]
            [13.363  2.869 -2.683]>
