...
from .representations.core import AbstractRepresentation
...
class AbstractVectorLike(
    ...
    def vconvert(
        self,
        target: AbstractRepresentation,  # type: ignore[type-arg]
        /,
        *args: Any,
        **kwargs: Any,
    ) -> "AbstractVectorLike":
    ...
    def __array_namespace__(self) -> Any:
        ...
        --------
        >>> import coordinax as cx
        >>> vec = cx.vecs.CartPos2D.from_([3, 4], "m")
        >>> ns = vec.__array_namespace__()
        >>> ns
        <module 'quaxed.numpy' from ...>

        >>> ns.multiply(vec, 2)
        CartPos2D(x=Quantity(6, unit='m'), y=Quantity(8, unit='m'))
    ...
    def __setitem__(self, k: Any, v: Any) -> NoReturn:
        ...
        >>> vec = cx.vecs.CartPos2D.from_([[1, 2], [3, 4]], "m")
        >>> try: vec[0] = u.Q(1, "m")
        ... except TypeError as e: print(e)
        CartPos2D is immutable.
...
def is_vectorlike(obj: Any, /) -> TypeIs[AbstractVectorLike]:
    ...
    >>> cxv.is_vectorlike(vec)
    True

    >>> space = cxv.KinematicSpace.from_(vec)
    >>> cxv.is_vectorlike(space)
    True

    >>> import coordinax.frames as cxf
    >>> coord = cxf.Coordinate.from_(vec, cxf.ICRS())
    >>> cxv.is_vectorlike(coord)
    True

    >>> cxv.is_vectorlike(42)
    False
...
@plum.dispatch
def vector(obj: AbstractVectorLike, /) -> AbstractVectorLike:
