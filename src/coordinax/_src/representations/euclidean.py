...
from . import checks
from .base import AbstractDimensionalFlag, AbstractFixedComponentsRep, AbstractRep
...
@plum.dispatch
def cartesian_rep(obj: Abstract0D, /) -> "Cart0D": return cart0d
...
@final
class Cart0D(AbstractFixedComponentsRep[ZeroDKeys, ZeroDDims], Abstract0D):
...
@plum.dispatch
def cartesian_rep(obj: Abstract1D, /) -> "Cart1D": return cart1d
...
@final
class Cart1D(AbstractFixedComponentsRep[Cart1DKeys, Cart1DDims], Abstract1D):
    pass
...
@final
class Radial1D(AbstractFixedComponentsRep[RadialKeys, Radial1DDims], Abstract1D):
    pass
...
@plum.dispatch
def cartesian_rep(obj: Abstract2D, /) -> "Cart2D": return cart2d
...
@final
class Cart2D(AbstractFixedComponentsRep[Cart2DKeys, Cart2DDims], Abstract2D):
    pass
...
@final
class Polar2D(AbstractFixedComponentsRep[PolarKeys, Polar2DDims], Abstract2D):
    pass
...
@plum.dispatch
def cartesian_rep(obj: Abstract3D, /) -> "Cart3D":
    return cart3d
...
@final
class Cart3D(AbstractFixedComponentsRep[Cart3DKeys, Cart3DDims], Abstract3D):
    pass
...
@final
class Cylindrical3D(AbstractFixedComponentsRep[CylindricalKeys, Cylindrical3DDims], Abstract3D):
    pass
...
@final
class Spherical3D(AbstractFixedComponentsRep[SphericalKeys, Spherical3DDims], AbstractSpherical3D):
...
@final
class LonLatSpherical3D(AbstractFixedComponentsRep[LonLatSphericalKeys, LonLatSpherical3DDims], AbstractSpherical3D):
...
@final
class LonCosLatSpherical3D(AbstractFixedComponentsRep[LonCosLatSphericalKeys, LonCosLatSpherical3DDims], AbstractSpherical3D):
...
@final
class MathSpherical3D(AbstractFixedComponentsRep[MathSphericalKeys, Spherical3DDims], AbstractSpherical3D):
...
class ProlateSpheroidal3D(AbstractFixedComponentsRep[ProlateSpheroidalKeys, ProlateSpheroidal3DDims], AbstractSpherical3D):
    ...
    _: KW_ONLY
    Delta: Annotated[Real[u.quantity.StaticQuantity["length"], ""], Is[lambda x: x.value > 0]]
...
@final
class PoincarePolar6D(AbstractFixedComponentsRep[PoincarePolarKeys, PoincarePolarDims]):
    pass
...
@plum.dispatch
def cartesian_rep(obj: AbstractND, /) -> "CartND":
    return cartnd
...
@final
@dataclass(frozen=True, slots=True)
class SpaceTimeEuclidean(AbstractRep[Ks, Ds], AbstractND):
    ...
    >>> import unxt as u
    >>> rep = cx.r.SpaceTimeEuclidean(cx.r.cart3d)
    ...
    _: KW_ONLY
    c: Float[u.Quantity["speed"], ""] = field(default=u.Quantity(299_792.458, "km/s"))
    """Speed of light, by default ``Quantity(299_792.458, "km/s")``."""

    @property
    def components(self) -> Ks:
        return cast(
            "Ks", ("ct", *self.spatial_kind.components)  # type: ignore[attr-defined]
        )
...
@plum.dispatch.multi((SpaceTimeEuclidean,))
def cartesian_rep(obj: SpaceTimeEuclidean, /) -> SpaceTimeEuclidean:
...
@final
class CartND(AbstractFixedComponentsRep[CartNDKeys, CartNDDims], AbstractND):
    pass
