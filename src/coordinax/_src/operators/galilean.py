...
class GalileanOp(AbstractCompositeOperator):
    ...
    ...         delta_t=u.Q(2.5, "Gyr"),
    ...         delta_q=cx.Spherical3D(r=u.Q(1, "km"), theta=u.Q(90, "deg"),
    ...                                 phi=u.Q(0, "rad") ) ),
    ...     velocity=cx.ops.Add(
    ...         cx.CartVel3D.from_([1, 2, 3], "km/s") )
    ... )
    >>> op
    GalileanOp(
      translation=Add( delta_t=..., delta_q=Spherical3D( ... ) ),
      velocity=Add(CartVel3D( ... ))
    )
...
def simplify(op: GalileanOp, /, **kwargs: Any) -> SimplifyOpR:
    ...
    >>> cxo.simplify(op)
    Add( delta_t=Q(..., 's'), delta_q=Cart3D(...) )
