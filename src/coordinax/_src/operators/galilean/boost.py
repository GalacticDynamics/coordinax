...
class GalileanBoostOperator(AbstractGalileanOperator):
    ...
    velocity : :class:`vector.CartesianVel3D`
        ...
        :meth:`vector.CartesianVel3D.constructor` to enable a variety
    ...
    Note that the velocity is a :class:`vector.CartesianVel3D`, which
    was constructed from a 1D array, using
    :meth:`vector.CartesianVel3D.constructor`. We can also construct it
    directly:

    >>> boost = cx.CartesianVel3D.constructor([1, 2, 3], "m/s")
    ...
    velocity: CartesianVel3D = eqx.field(converter=CartesianVel3D.constructor)
    """The boost velocity.

    This parameters uses :meth:`vector.CartesianVel3D.constructor` to
    ...
    def __call__(
        ...
        >>> q = cx.CartesianPos3D.constructor([0, 0, 0], "m")
        >>> t = Quantity(1, "s")
        >>> newq, newt = op(q, t)
        >>> newt
