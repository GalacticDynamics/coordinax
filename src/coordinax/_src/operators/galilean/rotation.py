...
class GalileanRotationOperator(AbstractGalileanOperator):
    ...
    >>> q = cx.CartesianPos3D.constructor(q)  # from the previous example
    >>> newq, newt = op(q, t)
    >>> newq.x
    ...
    def __call__(
        ...
        >>> q = cx.CartesianPos3D.constructor([1, 0, 0], "m")
        >>> t = Quantity(1, "s")
        >>> newq, newt = op(q, t)
        >>> newq.x
        ...
        rcart = CartesianPos3D.constructor(vec_matmul(self.rotation, vec))
