...
def apply_op(op: AbstractOperator, tau: Any, x: ArrayLike, /, **kw: Any) -> Array:
    ...
    return api.apply_op(op, tau, cxr.point, chart, x, **kw)
...
def apply_op(op: AbstractOperator, tau: Any, x: AbcQ, /, **kw: Any) -> AbcQ:
    ...
    role = api.guess_role(x)
    chart = api.guess_chart(x)
    return api.apply_op(op, tau, role, chart, x, **kw)
...
def apply_op(
    ...
    **kw: Any,
    ...
    >>> func = cxo.apply_op.invoke(cxo.Rotate, None, cxr.Point, cxc.Cart3D, u.Q)
    >>> func(op, None, cxr.point, cxc.cart3d, q)
    Quantity(Array([0, 1, 0], dtype=int64), unit='km')

    >>> func = cxo.apply_op.invoke(cxo.Rotate, None, cxr.PhysDisp, cxc.Cart3D, u.Q)
