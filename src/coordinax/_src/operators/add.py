...
class Add(AbstractOperator):
    ...
    Note that the translation is a {class}`unxt.Quantity` and a
    {class}`coordinax.vecs.AbstractPos`, which was constructed from a 1D array,
    using :meth:`coordinax.vecs.AbstractPos.from_`.  We can also construct it
    directly, which allows for other vector types.

    >>> qshift = cx.Vector(
    ...     {"r": u.Q(1.0, "km"), "theta": u.Q(jnp.pi/2, "rad"), "phi": u.Q(0, "rad")},
    ...     rep=cx.r.sph3d, role=cx.r.pos)
    ...
    def operate(cls, params: dict[str, Any], arg: Any, /, **_: Any) -> Any:
        ...
        >>> params = {"delta": cxv.Cart3D.from_([1, 1, 1], "km")}
    ...
    def inverse(self) -> "Add":
        ...
        >>> qshift = cx.Vector.from_([1, 1, 1], "km")
