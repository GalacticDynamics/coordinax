...
@given(
    vec_and_chart=cxst.vectors_with_target_chart(
        chart=cxst.charts(), role=cx.roles.Point
    )
)
...
@settings(max_examples=10, deadline=None)
@given(
    vec_and_chart=cxst.vectors_with_target_chart(chart=cxst.charts(), role=cx.roles.Vel)
)
...
@settings(max_examples=10, deadline=None)
@given(
    vec_and_chart=cxst.vectors_with_target_chart(chart=cxst.charts(), role=cx.roles.Acc)
)
...
class TestPos:
    ...
    def test_pos_role_exists(self):
        """Test that Pos role is accessible."""
        assert hasattr(cx.charts.cart3d, "Pos")
        assert hasattr(cx.charts.cart3d, "pos")
        assert isinstance(cx.roles.pos, cx.roles.Pos)

    def test_pos_order(self):
        """Test that Pos has order 0 (same as Point)."""
        assert cx.roles.Pos.order == 0
        assert cx.roles.Point.order == 0

    def test_pos_derivative(self):
        """Test that derivative of Pos is Vel."""
        assert cx.roles.Pos.derivative() == cx.roles.Vel
    ...
    def test_create_pos_vector(self):
        ...
        assert isinstance(disp.role, cx.roles.Pos)
...
class TestIsEuclideanRep:
    ...
    def test_cartesian_is_euclidean(self):
        ...
        assert cx.charts.cart3d.is_euclidean is True
        assert cx.charts.cart2d.is_euclidean is True
...
class TestVectorAddition:
    ...
    def test_pos_plus_point_raises(self):
...
class TestAsDisplacement:
    ...
    def test_as_pos_with_rep_parameter(self):
        ...
        assert isinstance(disp_sph.role, cx.roles.Pos)
        assert disp_sph.chart == cx.charts.sph3d
        ...
        disp_cart = disp_sph.vconvert(cx.charts.cart3d, pos)
    ...
    def test_as_pos_from_non_cartesian(self):
        ...
        # from origin)
        assert disp.chart == cx.charts.cart3d
        assert isinstance(disp.role, cx.roles.Pos)
...
class TestVelVelAddition:
    ...
    def test_vel_plus_vel(self):
        """Test that Vel + Vel = Vel (same role addition)."""
        v1 = cx.Vector(
            {"x": u.Q(1.0, "m/s"), "y": u.Q(0.0, "m/s"), "z": u.Q(0.0, "m/s")},
            cx.charts.cart3d,
            cx.roles.vel,
        ...
        result = v1.add(v2)
        assert isinstance(result.role, cx.roles.Vel)
...
def test_displacement_addition_commutative():
    """Test that Displacement + Displacement is commutative."""
    d1 = cx.Vector(
        {"x": u.Q(1.0, "m"), "y": u.Q(2.0, "m"), "z": u.Q(3.0, "m")},
        cx.charts.cart3d,
        cx.roles.pos,
    ...
    assert isinstance(result1.role, cx.roles.Pos)
    assert isinstance(result2.role, cx.roles.Pos)
...
def test_displacement_transformation_via_tangent_transform():
    ...
    disp_cart = disp_at_phi0.vconvert(cx.charts.cart3d, pos_at_phi0)

    # At φ=0, ê_ρ = (cos 0, sin 0) = (1, 0)
    # So rho=1m should give dx=1m, dy=0m
    assert isinstance(disp_cart.role, cx.roles.Pos)
    ...
    disp_cart_90 = disp_at_phi90.vconvert(cx.charts.cart3d, pos_at_phi90)

    # At φ=90°, ê_ρ = (cos 90°, sin 90°) = (0, 1)
    # So rho=1m should give dx=0m, dy=1m
    assert isinstance(disp_cart_90.role, cx.roles.Pos)
...
def test_pos_plus_displacement_with_coordinate_conversion():
    """Test Pos + Displacement when representations differ.

    When adding Pos (in one rep) + Displacement (in another rep), the
    displacement must be converted using the position as the base point.

    IMPORTANT: Displacement uses physical components (uniform length units).
    """
...
def test_displacement_plus_displacement_different_reps_raises():
    """Test that Displacement + Displacement in different reps requires base point.
