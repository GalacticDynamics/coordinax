...
class TestAngleArithmetic:
    ...
    # prevent 2*x=inf overflow
    @given(
        angle=cxst.angles(elements=st.floats(min_value=-1e10, max_value=1e10, width=32))
    )
    def test_add_sub_roundtrip(self, angle: u.Angle) -> None:
        """(a + a) - a == a for non-overflowing values."""
        result = (angle + angle) - angle
        assert isinstance(result, u.Angle)
        assert jnp.allclose(result.value, angle.value, atol=1e-5)

    # Deliberately test overflow behavior with extreme values
    # Using values > max/2 ensures that doubling will overflow to infinity
    @given(
        angle=cxst.angles(
            elements=st.floats(
                min_value=1.9999999360571385e38,  # Exact float32, > max/2
                max_value=3.3999999521443642e38,  # Exact float32, near max
                width=32,
            )
        )
    )
    def test_add_overflow(self, angle: u.Angle) -> None:
        """Demonstrate that overflow produces infinity."""
        result = angle + angle
        assert isinstance(result, u.Angle)
        # This overflow is expected for extreme float32 values
...
class TestAnglesNamespace:
    """Tests for coordinax.angles namespace exports."""

    def test_parallax_in_namespace(self) -> None:
        """Test that Parallax is exported in coordinax.angles."""
        assert hasattr(cxa, "Parallax")

    def test_parallax_in_all(self) -> None:
        """Test that Parallax is in __all__."""
        assert "Parallax" in cxa.__all__
