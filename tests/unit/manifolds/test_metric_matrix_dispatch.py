...
def manifold_point_chart(request):
    ...
    if key == "euclidean_cart1d":
        return (cxm.R1, {"x": jnp.array(1.0)}, cxc.cart1d)
    if key == "euclidean_cart2d":
        return (cxm.R2, {"x": jnp.array(1.0), "y": jnp.array(2.0)}, cxc.cart2d)
    if key == "euclidean_cart3d":
        return (
            cxm.R3,
            {"x": jnp.array(1.0), "y": jnp.array(2.0), "z": jnp.array(3.0)},
            cxc.cart3d,
        )
    if key == "euclidean_cartnd":
        return (cxm.R3, {"q": jnp.array([1.0, 2.0, 3.0])}, cxc.cartnd)
    ...
    if key == "hyperspherical_sph2":
        return (
            cxm.S2,
            {"theta": jnp.array(jnp.pi / 2), "phi": jnp.array(0.0)},
            cxc.sph2,
        )
...
class TestMetricMatrixNumericalValues:
    ...
    def test_euclidean_cart2d_is_identity(self):
        pt = {"x": jnp.array(0.0), "y": jnp.array(0.0)}
    ...
    def test_euclidean_cart3d_is_identity(self):
        pt = {"x": jnp.array(1.0), "y": jnp.array(2.0), "z": jnp.array(3.0)}
    ...
    def test_minkowski_diagonal_signature(self):
        ...
        assert jnp.allclose(g.diagonal, jnp.array([-1.0, 1.0, 1.0, 1.0]))

    def test_hyperspherical_at_equator(self):
        """At theta=π/2: diag(S²) = (1, sin²(π/2)) = (1, 1)."""
        pt = {"theta": jnp.array(jnp.pi / 2), "phi": jnp.array(0.0)}
        g = cxmapi.metric_matrix(cxm.S2, pt, cxc.sph2)
        assert isinstance(g, DiagonalMetric)
        assert jnp.allclose(g.diagonal, jnp.array([1.0, 1.0]), atol=1e-6)

    def test_hyperspherical_at_off_equator(self):
        """At theta=π/3: diag(S²) = (1, sin²(π/3)) = (1, 3/4)."""
        pt = {"theta": jnp.array(jnp.pi / 3), "phi": jnp.array(0.0)}
        g = cxmapi.metric_matrix(cxm.S2, pt, cxc.sph2)
        assert isinstance(g, DiagonalMetric)
        expected = jnp.array([1.0, jnp.sin(jnp.pi / 3) ** 2])
...
class TestMetricMatrixJIT:
    ...
    def test_jit_euclidean_cart3d(self):
        ...
        result = compute(jnp.array(1.0), jnp.array(2.0), jnp.array(3.0))
    ...
    def test_jit_minkowski(self):
        ...
        result = compute(jnp.array(0.0), jnp.array(0.0), jnp.array(0.0), jnp.array(0.0))
        assert jnp.allclose(result, jnp.array([-1.0, 1.0, 1.0, 1.0]))
    ...
    def test_jit_hyperspherical(self):
        ...
        result = compute(jnp.array(jnp.pi / 2), jnp.array(0.0))
        assert jnp.allclose(result, jnp.array([1.0, 1.0]), atol=1e-6)
    ...
    def test_jit_cart2d(self):
        ...
        result = compute(jnp.array(0.0), jnp.array(0.0))
