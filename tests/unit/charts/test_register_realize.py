...
class TestPointTransformProlate:
    ...
    def test_the_refusal_names_the_components_that_were_bare(self) -> None:
        """Which two were bare is the useful half, and it differs by direction.

        Going out of the chart it is `mu`/`nu`; coming in it is `rho`/`z`. One
        shared message naming only the first pair sends a reader looking at the
        wrong components.
        """
        prolate = cxc.ProlateSpheroidal3D(Delta=u.StaticQuantity(2.0, "m"))
        with pytest.raises(ValueError, match=r"'rho' or 'z'"):
            cxc.pt_map({"rho": 2.0, "phi": 0.7, "z": 3.0}, cxc.cyl3d, prolate)
        with pytest.raises(ValueError, match=r"'mu' or 'nu'"):
            cxc.pt_map({"mu": 12.0, "nu": 0.5, "phi": 0.3}, prolate, cxc.cyl3d)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                cxc.sph3d,
                cxc.lonlat_sph3d,
                {"r": u.Q(1.0, "m"), "theta": u.Q(40.0, ""), "phi": u.Q(70.0, "")},
            ),
            (cxc.sph2, cxc.lonlat_sph2, {"theta": u.Q(40.0, ""), "phi": u.Q(70.0, "")}),
        ],
        ids=["sph3d", "sph2"],
    )
    def test_a_dimensionless_quantity_is_read_through_the_system_too(
        self, frm, mid, point
    ):
        """`Q(40, "")` in an angle slot is 40 of whatever `usys` calls an angle.

        It carries a unit, so it is not the bare path, but the unit says
        nothing about angles -- `uconvert_to_rad` documents this case and
        `complement_angle` has a branch for it that nothing else reaches.
        """
        kw = {"usys": u.unitsystem("m", "deg")}
        out = cxc.pt_map(point, frm, mid, **kw)
        assert float(u.ustrip("deg", out["lat"])) == pytest.approx(50.0)
        back = cxc.pt_map(out, mid, frm, **kw)
        assert float(u.ustrip("deg", back["theta"])) == pytest.approx(40.0)
