...
class TestTangentMapSameChart:
    ...
    def test_same_chart_identity(self) -> None:
        ...
        v = {"r": jnp.array(1), "theta": jnp.array(0), "phi": jnp.array(0)}
        at = {"r": jnp.array(2), "theta": jnp.array(0.5), "phi": jnp.array(0.5)}
...
class TestTangentMapCart3dToSph3d:
    ...
    def test_phi_direction_vector(self) -> None:
        ...
        np.testing.assert_allclose(result["r"], 0, atol=1e-6)
        np.testing.assert_allclose(result["theta"], 0, atol=1e-6)
    ...
    def test_z_direction_vector(self) -> None:
        ...
        v = {"x": jnp.array(0), "y": jnp.array(0), "z": jnp.array(1)}
        at = {"x": jnp.array(1), "y": jnp.array(0), "z": jnp.array(0)}
        result = cxr.tangent_map(
            v, cxc.cart3d, cxr.coord_disp, cxc.sph3d, at=at, usys=usys
        )
        np.testing.assert_allclose(result["r"], 0, atol=1e-6)
        np.testing.assert_allclose(result["theta"], -1, atol=1e-6)
        np.testing.assert_allclose(result["phi"], 0, atol=1e-6)
...
class TestTangentMapPhysicalBasis:
    ...
    def test_same_chart_identity_phys_basis(self) -> None:
        ...
        v = {"r": jnp.array(2), "theta": jnp.array(-1.5), "phi": jnp.array(0.25)}
        at = {"r": jnp.array(3), "theta": jnp.array(0.5), "phi": jnp.array(0.1)}
    ...
    def test_roundtrip_cart3d_sph3d_phys_disp(self) -> None:
        ...
        v_cart = {
            "x": u.Q(jnp.array(2), "m/s"),
            "y": u.Q(jnp.array(-1), "m/s"),
    ...
    def test_cconvert_7arg_with_phys_disp(self) -> None:
        ...
        v = {
            "x": u.Q(jnp.array(1), "m/s"),
            "y": u.Q(jnp.array(0), "m/s"),
            "z": u.Q(jnp.array(0), "m/s"),
        }
        at = {
            "x": u.Q(jnp.array(1), "m"),
            "y": u.Q(jnp.array(0), "m"),
...
class TestTangentMapCart2dToPolar2d:
    ...
    def test_at_45_deg(self) -> None:
        ...
        v = {"x": jnp.array(1), "y": jnp.array(0)}
        at = {"x": jnp.array(1), "y": jnp.array(1)}
        result = cxr.tangent_map(v, cxc.cart2d, cxr.coord_disp, cxc.polar2d, at=at)
        np.testing.assert_allclose(result["r"], 1 / jnp.sqrt(2), atol=1e-6)
        np.testing.assert_allclose(result["theta"], -1 / 2, atol=1e-6)
