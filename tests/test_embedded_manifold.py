...
from coordinax._src.representations.frames import frame_to_cart


def _embedded_twosphere(R: u.Quantity | None = None) -> cx.r.EmbeddedManifold:
    params = {} if R is None else {"R": R}
    return cx.r.EmbeddedManifold(
...
def test_twosphere_embed_pos_known_angles() -> None:
    ...
    q0 = cx.r.embed_pos(rep, p0)
    q1 = cx.r.embed_pos(rep, p1)
    q2 = cx.r.embed_pos(rep, p2)
...
def test_twosphere_project_pos_roundtrip_known() -> None:
    rep = _embedded_twosphere(u.Quantity(3.0, "km"))
...
def test_embed_project_roundtrip(theta: u.Angle, phi: u.Angle) -> None:
    rep = _embedded_twosphere(u.Quantity(1.0, "km"))
...
def test_tangent_basis_orthonormal(theta: u.Angle, phi: u.Angle) -> None:
    rep = _embedded_twosphere(u.Quantity(2.0, "km"))
    p = {"theta": theta, "phi": phi}

    B = frame_to_cart(rep, p)
...
def test_pushforward_pullback_consistency(
    ...
    p = {"theta": theta, "phi": phi}
    v = {
        "theta": u.Quantity(v_theta, "km/s"),
        "phi": u.Quantity(v_phi, "km/s"),
    }

    v_cart = cx.r.embed_dif(rep, v, p)
...
def test_vconvert_roundtrip_with_embedded(
    ...
    qvec = cx.Vector(data=p, rep=rep, role=cx.r.Pos())
    vvec = cx.Vector(data=v, rep=rep, role=cx.r.Vel())

    v_cart = vvec.vconvert(cx.r.cart3d, qvec)
