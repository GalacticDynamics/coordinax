...
def test_zero_width_bounds_is_refused() -> None:
    ...
    builder = cxfc.BishopBuilder(circle, "s", initial_normal="auto")
...
def test_s_max_does_not_disturb_an_in_bounds_query() -> None:
    ...
    """
    free = cxfc.BishopBuilder(cxfc.ArcLength(circle, "s"), "km", initial_normal="auto")
    pinned = cxfc.BishopBuilder(
        cxfc.ArcLength(circle, "s", s_max=u.Q(1.0, "km")), "km", initial_normal="auto"
    )
    x = _at_angle(0.5)
