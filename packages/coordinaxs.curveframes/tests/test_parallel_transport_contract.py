"""The contract every `AbstractParallelTransportTransform` must satisfy.
...
class TestInverse:
    ...
    def test_inverse_is_same_type(self, pt_case: SimpleNamespace) -> None:
        """`.inverse` returns the same transform type."""
        assert isinstance(pt_case.transform.inverse, type(pt_case.transform))
    ...
    def test_roundtrip_forward_inverse(
        ...
        fwd = pt_case.transform
        diff = p - fwd.location(tau)
        p_fwd = qnp.stack([qnp.sum(e * diff) for e in pt_case.fields(fwd, tau)])
    ...
    def test_double_inverse(self, pt_case: SimpleNamespace, tau_val: float) -> None:
        """`inverse.inverse` recovers the original frame fields."""
        tau = u.Q(tau_val, "s")
    ...
    def test_inverse_jit(self, pt_case: SimpleNamespace) -> None:
        """The inverse's location is JIT-compatible."""
        loc = jax.jit(pt_case.transform.inverse.location)(u.Q(0, "s"))
        assert loc.shape == (3,)
...
class TestJAX:
    """The transform composes with jit and vmap."""
...
class TestConstructors:
    """`from_` dispatches to `from_curve`, and the frame wraps the transform."""

    def test_from_dispatches_to_from_curve(
    ...
    def test_frame_direct_construction(self, pt_case: SimpleNamespace) -> None:
        ...
        xop = pt_case.transform
        frame = type(pt_case.frame)(
            base_frame=cxf.Alice(), xop=xop, xop_inv=xop.inverse
        )
        assert isinstance(frame.base_frame, cxf.Alice)
        assert isinstance(frame.xop, type(xop))

    def test_frame_from_curve_accepts_tau_unit(
        self, pt_case: SimpleNamespace, curve
    ) -> None:
        frame = type(pt_case.frame).from_curve(cxf.Alice(), curve, tau_unit="yr")
        assert frame.xop.tau_unit == u.unit("yr")

    def test_frame_xop_is_matching_transform(self, pt_case: SimpleNamespace) -> None:
        assert isinstance(pt_case.frame.xop, type(pt_case.transform))
...
class TestAct:
    ...
    def test_different_tau_gives_different_result(
        ...
        r1 = cxfm.act(pt_case.transform, u.Q(0, "s"), p)
        r2 = cxfm.act(pt_case.transform, u.Q(1, "s"), p)
