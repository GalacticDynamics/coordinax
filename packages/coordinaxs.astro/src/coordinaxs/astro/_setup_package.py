...
def coordinaxs_frames_exports() -> dict[str, object]:
    """Return frame symbols exported to ``coordinaxs.frames`` via entry-point."""
    try:
        ...
        # During `import coordinaxs.astro`, this provider may run while
        # `coordinaxs.astro._src.base_frame` is still being initialized.
        # Returning no exports here avoids failing the import cycle; exports
        # are loaded again once `coordinaxs.astro` initialization completes.
