...
This document is the **normative** specification for the mathematical and software design of `coordinaxs.astro`. The goals are:
...
$\mu$ is expressed in **magnitudes** (`mag`), a dimensionless logarithmic unit. All constructors and conversions in `coordinaxs.astro` enforce this unit constraint.
...
Because astronomical parallaxes are small ($p \ll 1$), the small-angle approximation $\tan p \approx p$ is often used, but `coordinaxs.astro` retains the exact $\tan$ form.
...
## `coordinaxs.hypothesis.astro`

!!! info `distance_moduli`

    `hypothesis` strategy for generating `coordinaxs.astro.DistanceModulus` instances.

!!! info `parallaxes`

    `hypothesis` strategy for generating `coordinaxs.astro.Parallax` instances.
