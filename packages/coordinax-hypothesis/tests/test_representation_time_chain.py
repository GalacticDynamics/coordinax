"""Tests for the representation_time_chain strategy."""

from coordinax_hypothesis import representation_time_chain, representations
from hypothesis import given, settings

import coordinax as cx


@given(chain=representation_time_chain(cx.r.Pos, cx.r.cart3d))
...
@given(chain=representation_time_chain(cx.r.Pos, cx.r.sph3d))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Pos, cx.r.radial1d))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Vel, cx.r.cart3d))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Vel, cx.r.polar2d))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Vel, cx.r.radial1d))
@settings(max_examples=30)
def test_radial_velocity_chain(chain: tuple) -> None:
    """Test 1D velocity chain structure."""
    assert len(chain) == 2
    vel_rep, pos_rep = chain
    assert isinstance(vel_rep, cx.r.Abstract1D)
    assert isinstance(pos_rep, cx.r.Abstract1D)


@given(chain=representation_time_chain(cx.r.Acc, cx.r.cart3d))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Acc, cx.r.sph3d))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Acc, cx.r.radial1d))
@settings(max_examples=30)
def test_radial_acceleration_chain(chain: tuple) -> None:
    """Test 1D acceleration chain structure."""
    assert len(chain) == 3
    acc_rep, vel_rep, pos_rep = chain
    assert isinstance(acc_rep, cx.r.Abstract1D)
    assert isinstance(vel_rep, cx.r.Abstract1D)
    assert isinstance(pos_rep, cx.r.Abstract1D)


@given(chain=representation_time_chain(cx.r.Pos, representations(filter=cx.r.Abstract3D)))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Vel, representations(filter=cx.r.Abstract3D)))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Acc, representations(filter=cx.r.Abstract3D)))
@settings(max_examples=30)
...
@given(chain=representation_time_chain(cx.r.Acc, representations(filter=cx.r.Abstract2D)))
@settings(max_examples=30)
def test_2d_acceleration_chain_preserves_dimensionality(chain: tuple) -> None:
    ...
    assert len(chain) == 3
    acc_rep, vel_rep, pos_rep = chain
    assert isinstance(acc_rep, cx.r.Abstract2D)
    assert isinstance(vel_rep, cx.r.Abstract2D)
    assert isinstance(pos_rep, cx.r.Abstract2D)
    assert acc_rep.dimensionality == 2
    assert vel_rep.dimensionality == 2
    assert pos_rep.dimensionality == 2
...
def test_3d_velocity_chain_preserves_dimensionality(chain: tuple) -> None:
    ...
    assert isinstance(vel_rep, cx.r.Abstract3D)
    assert isinstance(pos_rep, cx.r.Abstract3D)
    assert vel_rep.dimensionality == 3
    assert pos_rep.dimensionality == 3
