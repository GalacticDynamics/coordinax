"""Tests for the charts_like strategy."""

from hypothesis import given, settings

import coordinax as cx
import coordinax_hypothesis as cxst


@given(chart=cxst.charts_like(cx.charts.cart3d))
@settings(max_examples=50)
def test_charts_like_cart3d(chart: cx.charts.AbstractChart) -> None:
    """Test charts_like with Cart3D template."""
    assert isinstance(chart, cx.charts.Abstract3D)
    assert chart.ndim == 3


@given(chart=cxst.charts_like(cx.charts.sph3d))
@settings(max_examples=50)
def test_charts_like_spherical3d(chart: cx.charts.AbstractChart) -> None:
    """Test charts_like with Spherical3D template."""
    assert isinstance(chart, cx.charts.Abstract3D)
    assert isinstance(chart, cx.charts.AbstractSpherical3D)
    assert chart.ndim == 3


@given(chart=cxst.charts_like(cx.charts.polar2d))
@settings(max_examples=50)
def test_charts_like_polar2d(chart: cx.charts.AbstractChart) -> None:
    """Test charts_like with Polar2D template."""
    assert isinstance(chart, cx.charts.Abstract2D)
    assert chart.ndim == 2


@given(chart=cxst.charts_like(cx.charts.radial1d))
@settings(max_examples=50)
def test_charts_like_radial1d(chart: cx.charts.AbstractChart) -> None:
    """Test charts_like with Radial1D template."""
    assert isinstance(chart, cx.charts.Abstract1D)
    assert chart.ndim == 1


@given(chart=cxst.charts_like(cx.charts.twosphere))
@settings(max_examples=50)
def test_charts_like_twosphere(chart: cx.charts.AbstractChart) -> None:
    """Test charts_like with TwoSphere template."""
    assert isinstance(chart, cx.charts.Abstract2D)
    assert chart.ndim == 2
