...
@given(
    source_chart=t=cxst.charts(filter=cxc.Abstract3D),
    target_chart=t=cxst.charts_like(cxst.charts(filter=cxc.Abstract3D)),
)
...
The `chart_time_chain` strategy generates chains of time antiderivatives given a role flag and a chart, useful for testing conversions across acceleration → velocity → position:
...
The `vectors_with_target_chart` strategy generates a vector and a full chain of compatible target charts, perfect for testing conversions. The target chain automatically matches the flags of the source vector:
...
@given(
    ...
    point_vec=cxst.vectors(chart=t=cxc.cart3d, role=cxr.PhysDisp),
)
...
@given(
    ...
    point_vec=cxst.vectors(chart=t=cxc.cart3d, role=cxr.PhysDisp, shape=(5,)),
)
...
Use these strategies to test mathematical properties of coordinate transformations:
...
The {mod}`coordinax.hypothesis` package builds on top of {mod}`unxt-hypothesis` strategies. You can use both packages together:
...
@example(distance=cxd.Distance(0, "kpc"))  # Zero distance
@example(distance=cxd.Distance(1, "kpc"))  # Unit distance
def test_distance_properties_with_edge_cases(distance):
    """Test distance properties including edge cases."""
    assert isinstance(distance, cxd.Distance)
...
The `coordinax.hypothesis` package automatically registers strategies for distance types with Hypothesis's `st.from_type()` function. This allows you to use these types in function annotations and let Hypothesis automatically generate test values.
...
- `coordinax.distances.DistanceModulus`
- `coordinax.distances.Parallax`
