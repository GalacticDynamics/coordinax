...
Generate random `coordinax.distances.DistanceModulus` objects for testing. distance and angle strategiesategy builds on `unxt_hypothesis.quantities` to generate distance moduli (apparent minus absolute magnitude). Distance moduli are always in units of 'mag'. The strategy is useful for property-based testing of magnitude-based distance computations.
...
**Returns:** `SearchStrategy[coordinax.distances.DistanceModulus]`
...
Generate random `coordinax.distances.Parallax` objects for testing.
...
**Returns:** `SearchStrategy[coordinax.distances.Parallax]`
...
The {mod}`coordinax.hypothesis` package builds on top of {mod}`unxt-hypothesis` strategies.
