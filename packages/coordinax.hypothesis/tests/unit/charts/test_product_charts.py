"""Tests for product-chart generation via charts(...)."""
...
class TestProductChartsBasic:
    """Basic tests for product-chart generation."""
...
class TestProductChartsWithFixedFactors:
    """Tests for explicit CartesianProductChart factor inputs."""
...
class TestProductChartsFactorCount:
    ...
    @given(
        chart=cxst.charts(
            cxc.AbstractCartesianProductChart, min_factors=2, max_factors=2
        )
    )
    ...
    @given(
        chart=cxst.charts(
            cxc.AbstractCartesianProductChart, min_factors=3, max_factors=5
        )
    )
...
class TestProductChartsIntegration:
    ...
    def test_with_cdicts_strategy(
        ...
        data = pdict.draw(cxst.cdicts(chart))
...
def test_product_charts_with_ndim(ndim: int, chart: st.DataObject) -> None:
    ...
    product_chart = chart.draw(
        cxst.charts(cxc.AbstractCartesianProductChart, ndim=ndim)
    )
...
@given(
    data=st.data(),
    factor_charts=st.lists(
        st.sampled_from([cxc.cart1d, cxc.polar2d, cxc.cart3d]), min_size=2, max_size=3
    ),
)
def test_product_charts_with_fixed_factors_and_ndim(
    data: st.DataObject, factor_charts: list[cxc.AbstractChart]
) -> None:
    """Test charts(CartesianProductChart, ...) with fixed factors."""
    # Create product chart with fixed factors
    chart = data.draw(
        cxst.charts(cxc.CartesianProductChart, factor_charts=tuple(factor_charts))
    )
