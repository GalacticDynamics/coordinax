...
def charts_like(
    ...
    chart = draw(charts(filter=flags, ndim=template.ndim, exclude=exclude))  # ty: ignore[missing-argument]
    assume(can_pt_map(template, chart))
    assume(can_pt_map(chart, template))
    return chart
