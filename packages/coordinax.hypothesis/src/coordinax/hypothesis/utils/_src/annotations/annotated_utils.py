...
BeartypeValidator = beartype.vale.Is[lambda x: x].__class__  # type: ignore[attr-defined]
...
def strategy_for_annotation(
    ann: AnnotatedNotIntrospectable,  # type: ignore[type-arg]
    /,
    *,
    meta: Metadata,
) -> st.SearchStrategy:
    ...
    for md in ann.ann.__metadata__:
        if isinstance(md, dict):
            meta |= md  # type: ignore[typeddict-item]
    ...
    return strategy_for_annotation(wrap_if_not_inspectable(typ), meta=meta)  # type: ignore[arg-type]
