...
def cartesian_chart(obj: Any, /) -> "coordinax.charts.AbstractChart":
    r"""Return the corresponding Cartesian chart for a given chart.

    This function provides the canonical flat-space Cartesian chart associated
    with any coordinate system. It maps each chart to its natural Cartesian
    equivalent in the same dimensional space.

    Mathematical Definition:

    For a coordinate chart $\mathcal{R}$ in $n$-dimensional space, this returns
    the Cartesian chart $\mathcal{C}_n$ such that:
    ...
    The Cartesian chart uses orthonormal basis vectors with components typically
    denoted $(x)$, $(x, y)$, $(x, y, z)$, or $(q_1, \ldots, q_n)$ for arbitrary
    dimension.
    ...
    - This function does **not** perform coordinate transformation; it only
      returns the chart type. Use
      {func}`~coordinax.charts.pt_map` for actual coordinate
      conversion.
    ...
    - Intrinsic 2-sphere charts do not have a global Cartesian 2D chart and
...
@plum.dispatch.abstract
def guess_chart(*_: Any) -> "coordinax.charts.AbstractChart":
...
def cdict(_: Any, /) -> CDict:
    ...
    Keys are component names and values are corresponding scalars/arrays or
    quantities.
