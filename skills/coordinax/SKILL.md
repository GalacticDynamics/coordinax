...
| A tangent conversion returns `NaN` where the point converts fine | The base point is on a chart singularity — the origin or the z-axis in `sph3d`, where `d(phi)/dx = -y/(x^2+y^2)` is `0/0`. The _point_ has coordinates there; its _velocity_ has no components, so `NaN` is the answer, not a bug. Convert the tangent in a chart that is regular at that point, or keep the base point off the axis. |
